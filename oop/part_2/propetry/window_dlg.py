...
show() - для отображения окна на экране (выводит в консоль строку в
 формате: "<Заголовок>: <ширина>, <высота>", например "Диалог 1: 100, 50").
...
class WindowDlg:
    ...
    def width(self, width):
        if isinstance(width, int) and 0 < width < 10_000:
            self.__width = width
            self.show()
    ...
    def height(self, height):
        if isinstance(height, int) and 0 < height < 10_000:
            self.__height = height
            self.show()
