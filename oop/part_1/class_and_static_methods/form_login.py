"""В программе объявлен следующий класс для работы с формами ввода
логин/пароль:
class FormLogin:
    ...
    def render_template(self):
        return "\n".join(['<form action="#">', self.login.get_html(),
...
name - название для поля (сохраняет передаваемое имя, например, "Логин" или
"Пароль");
...
class TextInput:
    ...
    def get_html(self):
        return f"<p class='login'>{self.name}: <input type='text' size=" \
               f"{self.size} />"
...
class PasswordInput:
    ...
    def get_html(self):
        return f"<p class='password'>{self.name}: <input type='text' size=" \
               f"{self.size} />"
