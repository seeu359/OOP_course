...
где name - наименование планеты; diametr - диаметр планеты (любое положительное
число); period_solar - период (время) обращения планеты вокруг Солнца (любое
 положительное число); period - период обращения планеты вокруг своей оси
...
class SolarSystem:
    ...
    def __new__(cls, *args, **kwargs):
        if cls.solar_system is None:
            cls.solar_system = super().__new__(cls)
            return cls.solar_system
        return cls.solar_system
